-- Migration 0016 — scale the "called PKs" bonus & decouple it from outcome
--
-- New rules for KO matches:
--   * Outcome correct                              → STAGE_OUTCOME_POINTS
--   * Predicted PKs AND match went to PKs          → STAGE_OUTCOME_POINTS
--     (flat bonus, independent of whether the player picked the winner)
--   * Predicted regulation score exactly correct   → STAGE_EXACT_BONUS
--     (only when the player did NOT call PKs; outcome must also be correct)
--
-- The two bonuses stack: an R16 PK match where you called PKs + winner pays
-- 8 (winner) + 8 (PK call) = 16 pts.
...
begin
  ...
  ko_scored as (
    select
      ...
      -- Outcome points (unchanged): paid whenever the predicted side wins.
      ...
      -- PK-call bonus (NEW): flat stage-scaled bonus for calling penalties
      -- on a match that goes to PKs. Independent of outcome correctness.
      + sum(case
          when mp.predicted_penalties = true and kf.went_to_penalties = true
          then
            case kf.stage
              when 'R32'   then  5
              when 'R16'   then  8
      ...
      -- Exact-score bonus (unchanged in spirit): only when the player did
      -- NOT call PKs, the outcome is correct, and regulation scores match.
      + sum(case
          when mp.predicted_outcome = kf.actual_outcome
            and coalesce(mp.predicted_penalties, false) = false
      ...
      -- "Bonus events" earned in KO matches (counts toward the leaderboard
      -- exact-score tally): either the regulation-exact bonus, or the
      -- PK-call bonus.
      sum(case
        when (mp.predicted_outcome = kf.actual_outcome
              and coalesce(mp.predicted_penalties, false) = false
              and mp.predicted_home_score = kf.home_score
              and mp.predicted_away_score = kf.away_score)
          or (mp.predicted_penalties = true and kf.went_to_penalties = true)
...
-- Re-run for everyone so the new rules apply to any KO matches that already
-- finished (no-op today, but future-proof for re-runs).
