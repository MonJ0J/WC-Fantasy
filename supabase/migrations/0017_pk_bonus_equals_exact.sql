-- Migration 0017 — PK-call bonus equals the exact-score bonus
--
-- We over-corrected in migration 0016 by paying STAGE_OUTCOME_POINTS for a
-- correct PK call. That made "tick the box + pick a side" dominate every
-- KO match. New rule, same idea but smaller:
--
--   * Outcome correct                       → STAGE_OUTCOME_POINTS
--   * Predicted PKs AND match went to PKs   → STAGE_EXACT_BONUS  (NEW: was outcome points)
--   * Predicted exact regulation score      → STAGE_EXACT_BONUS  (unchanged; outcome must match too)
--
-- The PK-call bonus is still independent of whether the player picked the
-- winning side, but it's the same magnitude as nailing the score. Calling
-- PKs is now an *alternative path* to the same bonus, not a free upgrade.
...
begin
  ...
  ko_scored as (
    select
      ...
      -- Outcome points: paid whenever the predicted side wins.
      ...
      -- Bonus: either the player nailed the exact regulation score (and the
      -- outcome) OR they called PKs and the match went to PKs. Same value
      -- both ways; only one path per match (you can't do both at once
      -- because predicting PKs blanks the score).
      + sum(case
          when (
            mp.predicted_outcome = kf.actual_outcome
